...
class SemanticAnalyzer(private var program: Program) {
        ...
        classDecl.members.forEach { member ->
            when (member) {
                ...
                is MemberDecl.ConstructorDecl -> {
                    // Конструкторы пока не добавляем в отдельную таблицу
     ...
     * - Проверяет дублирование методов в классе (уже проверяется при добавлении)
     * - Проверяет дублирование параметров в методе (уже проверяется при добавлении)
     * - Проверяет дублирование локальных переменных (уже проверяется при добавлении)
     */
